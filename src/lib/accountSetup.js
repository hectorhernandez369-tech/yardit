...
export function isAccountSetupComplete(user) {
  ...
  return Boolean(
    !getNameValidationError(user.first_name, "First name") &&
    !getNameValidationError(user.last_name, "Last name") &&
    !getPhoneValidationError(user.phone || user.phone_number) &&
...
export function getProfileCompletionPercent(user) {
  ...
  const items = [
    Boolean(!getNameValidationError(user.first_name, "First name") && !getNameValidationError(user.last_name, "Last name")),
    Boolean(user.terms_accepted && user.privacy_accepted && user.community_guidelines_accepted),
    Boolean(!getPhoneValidationError(user.phone || user.phone_number)),
    hasConfirmedAddress(user),
