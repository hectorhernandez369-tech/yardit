  ...
  return <Card className="border-2 border-[#5DADA5]/30 shadow-sm">
    ...
    <CardContent className="space-y-4">
      ...
      <div className="flex flex-col gap-3 rounded-2xl bg-[#F3E6CF] p-4 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="font-bold text-[#2C4F4E]">Push permission: {pushStatusLabel(browserStatus)}</p><p className="text-xs text-slate-600">Bell/history notifications are always kept separately.</p></div>
        {browserStatus !== "enabled" && <Button onClick={handleEnablePush} disabled={enabling || browserStatus === "blocked" || browserStatus === "unsupported"} className="bg-[#5DADA5] text-white hover:bg-[#4A9B93]">{enabling && <Loader2 className="h-4 w-4 animate-spin" />} Enable Push Notifications</Button>}
      </div>
