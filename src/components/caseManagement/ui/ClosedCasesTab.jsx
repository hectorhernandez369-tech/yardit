...
export default function ClosedCasesTab({ user, searchResults, onOpenCase, refreshKey }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const allCases = await base44.entities.Case.filter({ status: "closed", assigned_admin_id: user.id });
    setCases(allCases);
  ...
  return (
    <div className="mt-4">
      <div className="mb-3 rounded-xl border-2 border-[#5DADA5] bg-teal-50 p-3">
        <p className="text-sm font-bold text-[#2C4F4E]">My Closed Cases</p>
        <p className="text-xs text-[#2C4F4E]">These are closed cases that were assigned to you.</p>
      </div>
      {displayed.length === 0 ? (
        <p className="text-gray-500 text-center py-8">No closed cases are assigned to you.</p>
      ) : (
        <>
