...
export default function StepTwo({ formData, setFormData, onGeocodeRef }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isNeighborhood && (formData.tier === "featured" || formData.tier === "premium") && (
        <div className="pt-6 mt-6 border-t-2 border-[#2C4F4E]/20">
          <div className="rounded-xl border-2 border-[#2C4F4E] bg-[#E7D7B8] p-4 mb-4">
            <h3 className="text-[#2C4F4E] font-semibold">Event Dates</h3>
            <p className="text-sm text-[#1F2937] opacity-80">
              ...
              {formData.selectedRangeStartDate && formData.selectedRangeEndDate && (
                 <p className={`text-xs mt-1 ${
                   (() => {
                     ...
                     const req = formData.tier === "featured" ? 3 : 5;
                     return diffDays === req ? "hidden" : "text-red-500 font-medium";
                   })()
                 }`}>
                   Must be exactly {formData.tier === "featured" ? 3 : 5} consecutive days.
                 </p>
              )}
            </div>
          </div>
