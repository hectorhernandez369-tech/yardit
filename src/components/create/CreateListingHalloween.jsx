    ...
    return (
      <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-purple-200 bg-purple-50/70 p-4 space-y-3">
          ...
          {candyPrimary ? <div className="flex w-full items-center gap-2 rounded-xl border border-orange-400 bg-orange-50 p-3 text-sm text-orange-950"><Candy className="h-4 w-4" /><span><strong>Candy stop</strong><span className="block text-[11px] font-normal opacity-70">Candy is automatically expected for this Spot Type.</span></span></div> : !formData.halloween_tags?.includes("no_candy_here") && <button type="button" onClick={() => setFormData((prev) => ({ ...prev, halloween_candy_available: prev.halloween_candy_available === true ? false : true, halloween_candy_schedule_mode: prev.halloween_candy_available === true ? "" : (prev.halloween_candy_schedule_mode || "halloween_only") }))} className={`flex w-full items-center gap-2 rounded-xl border p-3 text-left text-sm ${formData.halloween_candy_available ? "border-orange-500 bg-orange-50 text-orange-950" : "border-slate-200 bg-white text-slate-700"}`}><Candy className="h-4 w-4" /><span><strong>Candy Available at This Spot</strong><span className="block text-[11px] font-normal opacity-70">Keep your main Halloween Spot type and tell visitors when candy is available.</span></span></button>}

          {(candyPrimary || formData.halloween_candy_available) && <div className="rounded-xl border border-orange-200 bg-white p-3 space-y-3">
            <div><Label className="text-sm font-black text-orange-950">When will candy be available?</Label><p className="text-[11px] text-slate-500">Choose Halloween night or enter your own candy dates and times.</p></div>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setFormData((prev) => ({ ...prev, halloween_candy_schedule_mode: "halloween_only", halloween_candy_start_date: trickOrTreatDate, halloween_candy_end_date: trickOrTreatDate }))} className={`rounded-lg border px-3 py-2 text-xs font-bold ${formData.halloween_candy_schedule_mode !== "custom" ? "border-orange-500 bg-orange-50 text-orange-950" : "border-slate-200 bg-white text-slate-700"}`}>Halloween Only — Oct. 31</button>
              <button type="button" onClick={() => setFormData((prev) => ({ ...prev, halloween_candy_schedule_mode: "custom" }))} className={`rounded-lg border px-3 py-2 text-xs font-bold ${formData.halloween_candy_schedule_mode === "custom" ? "border-purple-500 bg-purple-50 text-purple-950" : "border-slate-200 bg-white text-slate-700"}`}>Custom Dates & Times</button>
            </div>
            {formData.halloween_candy_schedule_mode === "custom" ? <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1"><Label className="text-xs">Candy start date</Label><Input type="date" value={formData.halloween_candy_start_date || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_start_date: e.target.value }))} /></div>
              <div className="space-y-1"><Label className="text-xs">Candy end date</Label><Input type="date" min={formData.halloween_candy_start_date || undefined} value={formData.halloween_candy_end_date || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_end_date: e.target.value }))} /></div>
              <div className="space-y-1"><Label className="text-xs">Candy starts</Label><Input type="time" value={formData.halloween_candy_start_time || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_start_time: e.target.value }))} /></div>
              <div className="space-y-1"><Label className="text-xs">Candy ends</Label><Input type="time" value={formData.halloween_candy_end_time || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_end_time: e.target.value }))} /></div>
            </div> : <div className="space-y-2">
              <Label className="text-xs font-semibold">Halloween time slot</Label>
              <div className="flex flex-wrap gap-2">
                {[["16:00","18:00","4–6 PM"],["17:00","19:00","5–7 PM"],["17:00","20:00","5–8 PM"],["18:00","20:00","6–8 PM"],["18:00","21:00","6–9 PM"],["18:00","22:00","6–10 PM"]].map(([start,end,label]) => {
                  const active = formData.halloween_candy_start_time === start && formData.halloween_candy_end_time === end;
                  return <button key={label} type="button" onClick={() => setFormData((prev) => ({ ...prev, halloween_candy_schedule_mode: "halloween_only", halloween_candy_start_date: trickOrTreatDate, halloween_candy_end_date: trickOrTreatDate, halloween_candy_start_time: start, halloween_candy_end_time: end, halloween_candy_custom_time: false }))} className={`rounded-full border px-3 py-1.5 text-xs font-bold ${active ? "border-orange-500 bg-orange-100 text-orange-950" : "border-slate-200 bg-white text-slate-700"}`}>{label}</button>;
                })}
                <button type="button" onClick={() => setFormData((prev) => ({ ...prev, halloween_candy_schedule_mode: "halloween_only", halloween_candy_start_date: trickOrTreatDate, halloween_candy_end_date: trickOrTreatDate, halloween_candy_custom_time: true }))} className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-xs font-bold text-slate-700">Custom time</button>
              </div>
              {formData.halloween_candy_custom_time && <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><Label className="text-xs">Candy starts</Label><Input type="time" value={formData.halloween_candy_start_time || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_start_time: e.target.value }))} /></div>
                <div className="space-y-1"><Label className="text-xs">Candy ends</Label><Input type="time" value={formData.halloween_candy_end_time || ""} onChange={(e) => setFormData((prev) => ({ ...prev, halloween_candy_end_time: e.target.value }))} /></div>
              </div>}
            </div>}
