...
import { LEAGUE_GAME_STATUSES, formatGameDate, formatGameTime, getLeagueGameDisplayStatus, normalizeLeagueGame, sortLeagueGames } from "./leagueGameUtils";
          ...
          {sortedGames.length === 0 ? (
            ...
            <div className="overflow-x-auto rounded-xl border">
              <table className="w-full min-w-[860px] text-xs">
                ...
                <tbody>{filteredGames.map((game) => { const displayStatus = getLeagueGameDisplayStatus(game); return <tr key={game.id} className="border-t align-top">{canManageSchedule && <td className="px-2 py-2"><input type="checkbox" aria-label={`Select ${game.game_title || "game"}`} checked={selectedGameIds.includes(game.id)} onChange={() => toggleGame(game.id)} className="h-4 w-4 accent-[#006168]" /></td>}<td className="px-2 py-2 whitespace-nowrap">{game.notes || ""}</td><td className="px-2 py-2 whitespace-nowrap font-semibold">{game.division || game.age_group}</td><td className="px-2 py-2"><div className="font-bold leading-tight">{game.home_team || "TBD"}</div><div className="text-slate-500 leading-tight">vs {game.away_team || "TBD"}</div><div className="mt-1 font-mono text-[10px] text-slate-400 break-all">Game ID: {game.id}</div></td><td className="px-2 py-2 whitespace-nowrap">{formatGameDate(game.game_date)}</td><td className="px-2 py-2 whitespace-nowrap">{formatGameTime(game.start_time)}</td><td className="px-2 py-2 max-w-[140px] truncate">{game.field_name || game.location || "—"}</td><td className="px-2 py-2">{canManageSchedule ? <Select value={displayStatus} onValueChange={(value) => updateStatus(game, value)}><SelectTrigger className="h-8 w-28 text-xs"><SelectValue /></SelectTrigger><SelectContent>{LEAGUE_GAME_STATUSES.map((status) => <SelectItem key={status} value={status}>{status}</SelectItem>)}</SelectContent></Select> : <span className="capitalize">{displayStatus}</span>}</td><td className="px-2 py-2 whitespace-nowrap font-bold">{Number(game.home_score || 0)} - {Number(game.away_score || 0)}</td><td className="px-2 py-2"><div className="flex flex-nowrap gap-1">{canEditGame(game) && <Button size="sm" variant="outline" className="h-7 px-2 text-xs" onClick={() => setEditingGame(game)}>Edit</Button>}{canManageSchedule && <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => duplicateGame(game)}><Copy className="h-3 w-3" /></Button>}{canManageSchedule && <Button size="sm" variant="outline" className="h-7 w-7 p-0" onClick={() => removeGame(game)}><Trash2 className="h-3 w-3 text-red-600" /></Button>}</div></td></tr>; })}</tbody>
              </table>
            </div>
          )}
        </CardContent>
