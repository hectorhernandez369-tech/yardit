...
export default function AcceptedTeamAssignmentCard({ request, account, teams, assignments, memberships, onAssigned }) {
  ...
  const assignTeam = async () => {
    ...
    if (!team) return toast.error("Choose an official team.");
  ...
  return <div className="rounded-xl border p-3 text-sm space-y-3">
    ...
    <div className="flex flex-col gap-2 sm:flex-row"><Select value={teamId} onValueChange={setTeamId}><SelectTrigger><SelectValue placeholder="Choose official team" /></SelectTrigger><SelectContent>{teams.map((team) => <SelectItem key={team.id} value={team.id}>{team.team_name} · {team.division || "No division"}</SelectItem>)}</SelectContent></Select><Button onClick={assignTeam} disabled={!teamId} className="shrink-0 bg-[#006168] text-white hover:bg-[#004f55]">Assign Team</Button></div>
  </div>;
}