...
export default function LeagueConnectionsTab({ account, user, accounts = [], isOwner, onRefresh }) {
  ...
  const assignTeam = async () => {
    const team = teams.find((item) => item.id === assignTeamId);
  ...
  return (
    <div className="space-y-4">
      ...
      {isOwner && <Card className="rounded-2xl bg-white"><CardHeader><CardTitle className="flex items-center gap-2 text-[#2C4F4E]"><ShieldCheck className="h-5 w-5" /> Assign Teams & Editing Permissions</CardTitle></CardHeader><CardContent className="grid gap-2 lg:grid-cols-[1fr_1fr_auto_auto_auto]"><Select value={assignRequestId} onValueChange={setAssignRequestId}><SelectTrigger><SelectValue placeholder="Approved request" /></SelectTrigger><SelectContent>{incomingRequests.filter((request) => request.status === "approved").map((request) => <SelectItem key={request.id} value={request.id}>{request.organization_name} · {request.requesting_email}</SelectItem>)}</SelectContent></Select><Select value={assignTeamId} onValueChange={setAssignTeamId}><SelectTrigger><SelectValue placeholder="Official team" /></SelectTrigger><SelectContent>{teams.map((team) => <SelectItem key={team.id} value={team.id}>{team.team_name} · {team.division || "No division"}</SelectItem>)}</SelectContent></Select><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={assignEdit} onChange={(e) => setAssignEdit(e.target.checked)} /> Edit games</label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={assignScore} onChange={(e) => setAssignScore(e.target.checked)} /> Submit scores</label><Button onClick={assignTeam} className="bg-[#F4A849] text-[#2C4F4E] hover:bg-[#E39635]">Assign</Button></CardContent></Card>}
