...
export default function LeagueTeamsTab({ account, user }) {
  ...
  const addMapping = async () => {
    ...
    const games = await base44.entities.LeagueGame.filter({ vendor_account_id: account.id });
    ...
    toast.success(updates.length ? `Mapping saved and ${updates.length} existing games linked.` : "Team name mapping saved for future imports.");
    refetchMappings();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="rounded-2xl border-slate-200 shadow-sm bg-white"><CardHeader><CardTitle className="text-[#2C4F4E]">Imported Team Name → Official Team ID</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-2 lg:grid-cols-[1fr_1fr_auto]"><Select value={mapping.team_id} onValueChange={(team_id) => setMapping({ ...mapping, team_id })}><SelectTrigger><SelectValue placeholder="Official team" /></SelectTrigger><SelectContent>{teams.map((item) => <SelectItem key={item.id} value={item.id}>{item.team_name}</SelectItem>)}</SelectContent></Select><Input placeholder="Imported name, e.g. Lindsay Cardinals or LYFC" value={mapping.imported_name} onChange={(e) => setMapping({ ...mapping, imported_name: e.target.value })} /><Button onClick={addMapping} className="bg-[#F4A849] text-[#2C4F4E] hover:bg-[#E39635]">Save Mapping</Button></div>{mappings.length === 0 ? <p className="text-sm text-slate-600">Map schedule names such as LYFC to the official team. Existing and future games will use that team ID for program access.</p> : <div className="flex flex-wrap gap-2">{mappings.map((item) => <Badge key={item.id} variant="secondary">{item.imported_name}</Badge>)}</div>}</CardContent></Card>
    </div>
  );
}