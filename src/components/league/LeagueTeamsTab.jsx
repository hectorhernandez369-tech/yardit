    ...
    <div className="space-y-4">
      ...
      <Card className="rounded-2xl border-slate-200 shadow-sm bg-white">
        ...
        <CardContent className="space-y-4">
          ...
          {teams.length === 0 ? <p className="text-sm text-slate-600">No official teams yet. Upload a schedule and Yardit can create or match them during import.</p> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{teams.map((item) => <div key={item.id} className="rounded-xl border p-4 space-y-2"><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-[#2C4F4E]">{item.team_name}</p><p className="text-[10px] font-mono text-slate-400 break-all">Team ID: {item.id}</p><p className="text-xs text-slate-600">{item.town_name || "Town not listed"} · {item.division || "Division not listed"} · {item.season || "Season not listed"}</p></div>{connectedTeamIds.has(item.id) ? <Badge className="bg-green-100 text-green-800">Connected</Badge> : <Badge variant="outline">No team account</Badge>}</div><p className="flex items-center gap-1 text-xs text-slate-500"><Gamepad2 className="h-3.5 w-3.5" /> {gameCountByTeam.get(item.id) || 0} scheduled game{(gameCountByTeam.get(item.id) || 0) === 1 ? "" : "s"}</p></div>)}</div>}
        </CardContent>
      </Card>
