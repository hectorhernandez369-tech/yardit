...
export default function OrganizerScoreGameCard({ account, game, onRefresh }) {
  ...
  const [draft, setDraft] = useState({ home_score: game.home_score ?? "", away_score: game.away_score ?? "", status: game.status || "upcoming" });
  useEffect(() => setDraft({ home_score: game.home_score ?? "", away_score: game.away_score ?? "", status: game.status || "upcoming" }), [game.home_score, game.away_score, game.status]);
  const save = async () => {
