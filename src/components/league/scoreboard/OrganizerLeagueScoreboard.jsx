...
export default function OrganizerLeagueScoreboard({ account, games = [], onRefresh }) {
  ...
  useEffect(() => {
    ...
    const pastGames = games.filter((game) => game.status === "upcoming" && game.game_date && game.game_date < today && !syncingIds.current.has(game.id));
    if (!pastGames.length) return;
    pastGames.forEach((game) => syncingIds.current.add(game.id));
    Promise.all(pastGames.map((game) => base44.functions.invoke("leagueGameAction", { action: "update_game", league_game_id: game.id, actor_account_id: account.id, actor_account_name: account.business_name, updates: { status: "final" } }))).then(() => onRefresh?.());
