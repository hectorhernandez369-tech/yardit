...
import { EVENTS_EXPERIENCE, RESIDENTIAL_EXPERIENCE, setPendingExperience, YARDIT_EVENTS_LOGO_URL, YARDIT_LOGO_URL } from "@/lib/experience";
      ...
      <Card className="w-full max-w-4xl overflow-hidden border-2 border-[#2C4F4E]/20 bg-white shadow-2xl">
        <CardContent className="p-5 sm:p-8">
          <div className="mb-6 text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">Choose your starting experience. You can switch later from your profile.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <button onClick={() => choose(EVENTS_EXPERIENCE)} className="rounded-3xl border-2 border-[#F4A849]/50 bg-slate-950 p-5 text-left text-white transition hover:border-[#F4A849] hover:shadow-lg">
              <img src={YARDIT_EVENTS_LOGO_URL} alt="Yardit Events" className="mb-4 h-16 w-auto object-contain" />
              <div className="mb-3 flex items-center gap-2 text-xl font-black text-[#F4A849]"><CalendarDays className="h-5 w-5" /> Yardit Events</div>
              <p className="text-sm text-white/80">Manage vendors, events, leagues, teams, schedules, and organizer dashboards in a separate events workspace.</p>
              <span className="mt-5 flex w-full items-center justify-center gap-2 rounded-md bg-[#F4A849] px-4 py-2 text-center text-sm font-semibold text-[#2C4F4E]"><Store className="h-4 w-4" /> Continue with Yardit Events</span>
            </button>
          </div>
        </CardContent>
      </Card>
