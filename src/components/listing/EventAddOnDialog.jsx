      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-5">
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
            <EventAddOnCard id="premium_visibility" title="Be Seen By More People" previewData={formData} price={RESIDENTIAL_EVENT_ADD_ONS.premium_visibility.price} selected={!!selectedAddOns.premium_visibility || !!existingAddOns.premium_visibility} onToggle={(checked) => !existingAddOns.premium_visibility && updateAddOns({ premium_visibility: checked })} description={<>{renderAlreadyActive("premium_visibility")}</>} />
            <EventAddOnCard id="animation" title="Animation" previewData={formData} price={RESIDENTIAL_EVENT_ADD_ONS.animation.price} selected={!!selectedAddOns.animation || !!existingAddOns.animation} onToggle={(checked) => !existingAddOns.animation && setFormData((prev) => ({ ...prev, event_animation: checked ? (prev.event_animation || "pulse") : "", event_add_ons: { ...(prev.event_add_ons || {}), animation: checked } }))} description={<>{renderAlreadyActive("animation")}</>}>
            ...
            <EventAddOnCard id="flyer_upload" title="Flyer Upload" previewData={formData} price={RESIDENTIAL_EVENT_ADD_ONS.flyer_upload.price} selected={!!selectedAddOns.flyer_upload || !!existingAddOns.flyer_upload} onToggle={(checked) => !existingAddOns.flyer_upload && updateAddOns({ flyer_upload: checked })} description={<>{renderAlreadyActive("flyer_upload")}</>}>
            ...
            <EventAddOnCard id="photo_gallery" title="Photo Gallery" previewData={formData} price={RESIDENTIAL_EVENT_ADD_ONS.photo_gallery.price} selected={!!selectedAddOns.photo_gallery || !!existingAddOns.photo_gallery} onToggle={(checked) => !existingAddOns.photo_gallery && setFormData((prev) => ({ ...prev, event_photo_gallery_count: checked ? (prev.event_photo_gallery_count || 1) : 0, event_add_ons: { ...(prev.event_add_ons || {}), photo_gallery: checked } }))} description={<>{renderAlreadyActive("photo_gallery")}</>}>
            ...
            <EventAddOnCard id="custom_icon" title="Custom Icon" previewData={formData} price={RESIDENTIAL_EVENT_ADD_ONS.custom_icon.price} selected={!!selectedAddOns.custom_icon || !!existingAddOns.custom_icon} onToggle={(checked) => !existingAddOns.custom_icon && updateAddOns({ custom_icon: checked })} description={<>{renderAlreadyActive("custom_icon")}</>}><EventIconManager tier="premium" selectedIcon={formData.event_icon} setSelectedIcon={(icon) => setFormData((prev) => ({ ...prev, event_icon: icon, event_logo_url: "" }))} uploadedImageUrl={formData.event_logo_url || ""} setUploadedImageUrl={(url) => setFormData((prev) => ({ ...prev, event_logo_url: url }))} /></EventAddOnCard>
