...
function getFireworksEventIcon(listing, isSelected = false, opacity = 1) {
  ...
  const width = Math.max(size + 28, 86);
  const height = size + 18;
  const html = `<style>@keyframes yarditFireworkBurst{0%{transform:scale(.2);opacity:0}18%{opacity:1}70%{opacity:.9}100%{transform:scale(1.45);opacity:0}}@keyframes yarditFireworkSpark{0%,100%{transform:translateY(0) scale(.92);opacity:.78}50%{transform:translateY(-3px) scale(1.08);opacity:1}}</style><div style="position:relative;width:${width}px;height:${height}px;opacity:${opacity};filter:drop-shadow(0 3px 8px rgba(15,23,42,.38));"><div style="position:absolute;left:50%;top:0;width:${size}px;height:${size}px;transform:translateX(-50%);"><div style="position:absolute;inset:8px;border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.96) 0 12%,rgba(250,204,21,.9) 13% 20%,rgba(244,63,94,.84) 21% 34%,rgba(59,130,246,.78) 35% 48%,rgba(59,130,246,0) 60%);animation:yarditFireworkBurst 1.35s ease-out infinite;"></div><div style="position:absolute;inset:3px;border-radius:999px;background:radial-gradient(circle,rgba(255,255,255,.94) 0 9%,rgba(96,165,250,.82) 10% 21%,rgba(168,85,247,.68) 22% 38%,rgba(168,85,247,0) 62%);animation:yarditFireworkBurst 1.55s ease-out infinite .38s;"></div><div style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);font-size:${Math.round(size * .46)}px;line-height:1;animation:yarditFireworkSpark 1.1s ease-in-out infinite;">🎆</div></div><div style="position:absolute;left:50%;bottom:0;transform:translateX(-50%);white-space:nowrap;border:1px solid rgba(17,24,39,.85);border-radius:999px;background:rgba(255,255,255,.96);padding:1px 6px;font-size:9px;font-weight:800;color:#111827;letter-spacing:.02em;">${label}</div></div>`;
  return makeDivIcon(`event_fireworks_animated_${listing?.id || "event"}_${isSelected}_${opacity}_labeled`, html, width, height, width / 2, size / 2);
}
