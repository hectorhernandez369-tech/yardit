...
export default function MarqueeBoard({ listing, point, onClose, onViewDetails }) {
  ...
  return (
    ...
    >
      <div className="relative w-[114px] rounded-lg border border-[#f4a849] bg-gradient-to-b from-[#7c2d12] to-[#3f1d0b] p-1.5 text-white shadow-[0_6px_14px_rgba(0,0,0,0.28)]">
        ...
        <div className="pointer-events-none absolute bottom-1.5 left-[-3px] top-1.5 flex flex-col justify-between">
          {Array.from({ length: 6 }).map((_, index) => (
            <span key={`left-bulb-${index}`} className="h-1.5 w-1.5 rounded-full bg-[#FFE08A] shadow-[0_0_4px_rgba(255,220,120,0.8)]" />
          ))}
        </div>
        <div className="pointer-events-none absolute bottom-1.5 right-[-3px] top-1.5 flex flex-col justify-between">
          {Array.from({ length: 6 }).map((_, index) => (
        ...
        <div className="pr-4 pt-1 text-center text-[7px] font-black uppercase leading-tight tracking-[0.03em] break-words">
          {title}
        </div>

        <div className="mt-1.5 grid gap-1">
          {slots.map((slot) => (
            <div key={slot.id} className="flex items-center justify-between gap-1 rounded-sm bg-white/10 px-1 py-1 text-[6px] leading-tight">
        ...
        <Button
          ...
          className="mt-1.5 h-5 w-full bg-amber-600 px-1 py-0 text-[7px] hover:bg-amber-700"
        >
          View More Details
        </Button>
      </div>
