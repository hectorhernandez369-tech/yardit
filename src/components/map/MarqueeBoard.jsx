...
export default function MarqueeBoard({ listing, point, onClose, onViewDetails }) {
  ...
  return (
    ...
    >
      <div className="relative w-[114px] rounded-lg border border-[#f4a849] bg-gradient-to-b from-[#7c2d12] to-[#3f1d0b] px-1.5 pb-1 pt-1 text-white shadow-[0_6px_14px_rgba(0,0,0,0.28)]">
        ...
        <div className="pointer-events-none absolute bottom-1 left-[-3px] top-1 flex flex-col justify-between">
          {Array.from({ length: 6 }).map((_, index) => (
            <span key={`left-bulb-${index}`} className="h-1.5 w-1.5 rounded-full bg-[#FFE08A] shadow-[0_0_4px_rgba(255,220,120,0.8)]" />
          ))}
        </div>
        <div className="pointer-events-none absolute bottom-1 right-[-3px] top-1 flex flex-col justify-between">
          {Array.from({ length: 6 }).map((_, index) => (
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute right-1 top-1 rounded-full bg-black/25 px-1 py-0 text-[8px] font-bold text-white hover:bg-black/40"
        >
          X
        </button>

        <div className="pr-4 pt-0.5 text-center text-[7px] font-black uppercase leading-tight tracking-[0.03em] break-words">
          {title}
        </div>

        <div className="mt-1 grid gap-0.5">
          {slots.map((slot) => (
            <div key={slot.id} className="flex items-center justify-between gap-1 rounded-sm bg-white/10 px-1 py-0.5 text-[6px] leading-tight">
        ...
        <Button
          ...
          className="mt-1 h-4.5 w-full bg-amber-600 px-1 py-0 text-[7px] hover:bg-amber-700"
        >
          View More Details
        </Button>
      </div>
