...
export default function VendorPhotoGallery({ account, onRefresh }) {
  ...
  return (
    <Card className="border-[#2C4F4E]/15 bg-white shadow-sm overflow-hidden">
      <CardHeader className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        ...
        <label className="inline-flex w-full cursor-pointer items-center justify-center gap-2 rounded-md bg-[#5DADA5] px-3 py-2 text-sm font-medium text-white hover:bg-[#4A9B93] sm:w-auto">
          ...
          <div className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 md:grid-cols-3">
            {photos.map((url) => (
              <div key={url} className="group relative overflow-hidden rounded-2xl border bg-white">
                <img src={url} alt="Vendor gallery" className="h-36 w-full object-cover sm:h-40" />
