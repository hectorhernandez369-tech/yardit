  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[360px_1fr]">
      <div className="space-y-4">
        <Card><CardHeader><CardTitle>Vendor Portal Passcode</CardTitle></CardHeader><CardContent className="space-y-3">
      ...
      <div className="grid gap-3">
        {users.map((item) => <Card key={item.id}><CardContent className="p-4 space-y-3"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold">{item.first_name} {item.last_name}</p><p className="text-sm text-slate-600">{item.authorized_email}</p></div><div className="flex items-center gap-2"><Badge>{item.status}</Badge><Button variant="outline" size="sm" disabled={!isOwner} onClick={() => removeUser(item)}>Remove</Button></div></div>{pins.length > 0 && <div className="border-t pt-3"><p className="mb-2 text-xs font-semibold text-slate-500 uppercase">Assigned trucks</p><div className="flex flex-wrap gap-2">{pins.filter((pin) => pin.is_active !== false).map((pin) => <label key={pin.id} className="flex items-center gap-2 rounded-full border px-3 py-1 text-xs"><input type="checkbox" disabled={!isOwner} checked={(item.assigned_pin_ids || []).includes(pin.id)} onChange={() => toggleAssignedPin(item, pin.id)} />{pin.pin_name}</label>)}</div></div>}</CardContent></Card>)}
      </div>
    </div>
  );
}