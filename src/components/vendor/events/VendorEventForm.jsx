    ...
    <div className="space-y-6">
      ...
      <div className="border-t border-slate-100 pt-5 flex flex-wrap justify-between gap-3">
        <div className="flex flex-wrap gap-2">
          {(!isEditing || (form.status || event.status) === "draft") && <Button variant="outline" disabled={saving} onClick={() => saveEvent("draft")} className="text-slate-600">Save as Draft</Button>}
          {isEditing && showPublicFields && (form.status || event.status) !== "draft" && (
            <>
              <Button variant="outline" disabled={saving || (form.status || event.status) === "cancelled"} onClick={() => saveEvent("draft")} className="text-slate-600">Hide From Map</Button>
        ...
        {isEditing && (form.status || event.status) === "draft" ? (
          <Button disabled={saving} onClick={() => saveEvent("published")} className="bg-[#2C4F4E] text-white hover:bg-[#3d6b6a] font-semibold px-6 shadow-sm">
