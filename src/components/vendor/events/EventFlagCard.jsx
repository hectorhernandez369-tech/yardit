    ...
    <Card className={`rounded-2xl bg-white transition ${selected ? "border-[#5DADA5] ring-2 ring-[#5DADA5]/20" : "border-[#2C4F4E]/10"}`}>
      <CardContent className="p-4 flex items-center justify-between gap-3">
        <button type="button" onClick={onEdit} className="flex min-w-0 items-center gap-3 text-left">
          <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full border-2 border-[#2C4F4E] bg-[#F4A849]">
            {iconAsset ? (
              <img src={iconAsset} alt="" className="h-10 w-10 object-contain" />
            ) : (
              <span className="text-xl">{getEventFlagIcon(spot.icon_key)}</span>
            )}
          </span>
          <span className="min-w-0">
            <span className="block truncate text-base font-black text-[#2C4F4E]">{spot.title || spot.label || "Flag"}</span>
            <span className="block text-xs font-semibold text-slate-600">{getEventFlagIconLabel(spot.icon_key)}</span>
