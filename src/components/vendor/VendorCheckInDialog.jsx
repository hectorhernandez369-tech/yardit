...
import { canStarterCheckInToday, getVendorTierConfig } from "@/lib/vendorTiers";
...
export default function VendorCheckInDialog({ open, onOpenChange, account, pin, user, checkIns, onRefresh }) {
  ...
  const createCheckIn = async () => {
    ...
    if (account.vendor_tier === "starter") {
      const result = canStarterCheckInToday(checkIns.filter((item) => item.vendor_account_id === account.id));
      if (!result.allowed) {
    ...
    const end = new Date(start.getTime() + Number(form.hours || 4) * 60 * 60 * 1000);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div><Label>Duration</Label><Select value={form.hours} onValueChange={(value) => setForm({ ...form, hours: value })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="2">2 hours</SelectItem><SelectItem value="4">4 hours</SelectItem><SelectItem value="8">8 hours</SelectItem><SelectItem value="12">12 hours</SelectItem></SelectContent></Select></div>
