  ...
  return (
    <nav className={`yardit-mobile-bottom-nav ${showVendorHomeButton ? "yardit-mobile-bottom-nav-vendor-home" : ""}`} aria-label="Mobile navigation">
      ...
      {showVendorHomeButton && (
        <button onClick={() => window.location.assign("/VendorDashboard")} className={`yardit-mobile-nav-item yardit-mobile-nav-vendor-home ${isVendorDashboardActive ? "is-active" : ""}`}>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" side="top" sideOffset={12} className="z-[3001] w-52 rounded-xl shadow-xl border border-gray-200 bg-white/95 backdrop-blur-md p-1.5">
          ...
          <div className="h-px bg-gray-100 my-0.5" />
          <DropdownMenuItem onSelect={onLogout} className="flex items-center gap-2 rounded-md px-2 py-1.5 text-sm text-red-600 hover:bg-red-50 transition mt-1">
