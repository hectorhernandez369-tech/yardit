...
export function getFallbackListingEligibility(listing, sale) {
  if (!listing) return { ok: false, reason: 'Listing was not found.' };
  if (listing.ownerUserId !== sale?.ownerUserId) return { ok: false, reason: 'Listing is not owned by the Neighborhood Sale organizer.' };
