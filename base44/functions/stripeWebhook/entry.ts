...
async function saveTransaction(base44, event, object, metadata, transactionType, status, extra = {}) {
  ...
  const payload = {
    ...
    original_amount_cents: Number(metadata.original_amount || getAmountCents(object) || 0),
    discount_amount_cents: Number(metadata.promo_discount_amount || metadata.discount_amount || 0),
    final_amount_cents: Number(metadata.final_amount || getAmountCents(object) || 0),
    currency: object.currency || '',
    promo_code: metadata.promo_code || '',
    non_refund_acknowledged: metadata.non_refund_acknowledged === 'true' || metadata.non_refund_acknowledged === true,
    non_refund_acknowledged_at: metadata.non_refund_acknowledged_at || '',
    non_refund_acknowledged_by_user_id: metadata.non_refund_acknowledged_by_user_id || metadata.user_id || metadata.owner_user_id || '',
