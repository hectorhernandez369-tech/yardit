...
Deno.serve(async (req) => {
  try {
    ...
    for (const target of targets) {
      ...
      const notification = { userId: target.user_id, user_id: target.user_id, title: `${vendor.business_name || 'A vendor'} just checked in`, message: `${vendor.business_name || 'A vendor'} just checked in ${distance.toFixed(1)} miles from you.`, type: target.mode, recipient: target.mode === 'vendor_subscription' ? 'users subscribed to a specific vendor' : 'users near a live vendor check-in', trigger: 'Vendor checks in at a public live location', delivery_methods: ['push', 'bell'], deep_link: `/?vendorCheckIn=${checkIn.id}`, dedupe_key: dedupeKey, registry_status: 'active', registry_version: '2026-08-24', related_entity_type: 'vendor_checkin', related_entity_id: checkIn.id, metadata: { dedupe_key: dedupeKey, vendor_account_id: checkIn.vendor_account_id, checkin_id: checkIn.id, url: `/?vendorCheckIn=${checkIn.id}` }, read: true, is_read: true };
      ...
      if (target.sub?.id && delivery.success) await base44.asServiceRole.entities.VendorNotificationSubscription.update(target.sub.id, { last_notified_checkin_id: checkIn.id, last_notified_at: new Date().toISOString(), updated_at: new Date().toISOString() });
      created++;
    }
